...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({ isOpen, onClose }) => {
  ...
  const testNotification = () => {
    // Use the createSystemNotification function
    createSystemNotification(
      t('notifications.messages.testNotification.title'),
      t('notifications.messages.testNotification.message'),
      'high'
    );
  };
  ...
  const notificationTypes = [
    ...
    {
      key: 'inactivityWarning' as const,
      title: 'Cảnh báo không hoạt động',
      description: 'Thông báo khi không có hoạt động trong thời gian dài',
      icon: <AiOutlineCoffee className="text-orange-500" />,
      color: 'orange'
    },
    {
      key: 'system' as const,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Inactivity Settings */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Cài đặt cảnh báo không hoạt động</h3>
            
            <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <AiOutlineCoffee className="text-orange-500" />
                  <div>
                    <p className="font-medium text-gray-900 dark:text-gray-100">Cảnh báo không hoạt động</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Nhận thông báo khi không có hoạt động trong thời gian dài</p>
                  </div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={config.types.inactivityWarning}
                    onChange={(e) => handleUpdateTypeConfig('inactivityWarning', e.target.checked)}
                    className="sr-only peer"
                  />
                  <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                </label>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Ngưỡng thời gian không hoạt động (phút)
                </label>
                <select
                  value={25} // Fixed at 25 minutes (Pomodoro length)
                  disabled={true}
                  className="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-70"
                >
                  <option value={25}>25 phút (1 Pomodoro)</option>
                </select>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Cảnh báo sẽ hiển thị khi bạn không thực hiện phiên Pomodoro nào trong 25 phút
                </p>
              </div>
            </div>
          </div>
